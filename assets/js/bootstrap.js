			...
			_proto._addTouchEventListeners = function _addTouchEventListeners() {
				...
				var end = function end(event) {
					...
					if (_this3._config.pause === "hover") {
						...
						// carousel would stop cycling until user tapped out of it; here, we
						// listen for touchend, explicitly pause the carousel (as if it's
						// the second time we tap on it, mouseenter compat event is NOT
						// fired) and after a timeout (to allow for mouse compatibility
		...
		(function () {
			...
			_proto._setScrollbar = function _setScrollbar() {
				...
				if (this._isBodyOverflowing) {
					...
					// not set
					//   while $(DOMNode).css('padding-right') returns the calculated
					...
					.map(function (element) {
						...
						if (target) {
							...
							if (targetBCR.width || targetBCR.height) {
								// TODO (fat): remove sketch reliance on jQuery
								// position/offset
								return [
			...
			_proto._activate = function _activate(target) {
				...
				if ($link.hasClass(ClassName$8.DROPDOWN_ITEM)) {
					...
					$link
						...
						.addClass(ClassName$8.ACTIVE); // Handle special case when
					// .nav-link is inside .nav-item
